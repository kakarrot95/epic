...
{
    ll x, y, f;
    Extend_Euler(ll x, ll y, ll f) : x(x), y(y), f(f){};
};
// solves ax + by = 1 
Extend_Euler extend_euler(ll a,ll b)
{
    if(a==0)
        return {0, 1, b};
    Extend_Euler ans = extend_euler(b % a, a);
    return {ans.y - (b / a) * ans.x, ans.x, ans.f};
}
...
void solve(){
    ...
    Extend_Euler ans = extend_euler(x, m);
    cout << (ans.x >= 0 ? ans.x : m + ans.x);
}
int main(){
    FAST
    int t=1;
